...
workflow {
    ...
    if ( params.help || params.bam_dir == false || params.output_dir == false || params.genome_fasta == false || params.fpscore_matrix == false ){
        ...
        bedFilesList = extractTFBSBeds(motifMatrix_ch).buffer( size: 110, remainder: true ).toList()//.view()
        ...
        // now we can run the variant-calling process in parallel for each bedFiles subchannel
